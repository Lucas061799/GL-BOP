      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div className={`mx-auto px-4 md:px-10 py-6 md:py-8 space-y-6 md:space-y-8 ${inQuoteFlow ? 'max-w-7xl' : 'max-w-5xl 2xl:max-w-6xl'}`}>
            ...
            {/* Form-review callout — sticks under the last section
                when the user is on the form pages and something is
                still missing. Same chip recipe as the Bind & Pay
                missing-items callout for consistency. */}
            {!inQuoteFlow && (() => {
              ...
              const incomplete = []
              if (!sm.classId) {
                incomplete.push({ id: 1, label: 'Pick a class code' })
              }
              const bizMissing = [
                !biz.name              && 'business name',
                !biz.entityType        && 'entity type',
                !biz.effectiveDate     && 'policy effective date',
                !biz.annualRevenue     && 'annual revenue',
                !biz.annualPayroll     && 'annual payroll',
                !biz.numberOfEmployees && 'full-time employees',
                !biz.phone             && 'phone',
                !biz.email             && 'email',
              ].filter(Boolean)
              if (bizMissing.length > 0) {
                incomplete.push({ id: 2, label: `Applicant Information · ${bizMissing.join(', ')}` })
              }
              const locMissing = [
                !loc.address && 'street address',
                !loc.city    && 'city',
                !loc.state   && 'state',
                !loc.zip     && 'zip',
              ].filter(Boolean)
              if (locMissing.length > 0) {
                incomplete.push({ id: 3, label: `Location · ${locMissing.join(', ')}` })
              }
              const hasCov = !!cov && Object.keys(cov).length > 0
              if (!hasCov) {
                incomplete.push({ id: 4, label: 'Coverage Limits — confirm or adjust the GL limit' })
              }
              const uwLeft = REQUIRED_UW.filter(k => uw[k] === undefined || uw[k] === null || uw[k] === '').length
              if (uwLeft > 0) {
                incomplete.push({ id: 5, label: `Underwriting Questions · ${uwLeft} unanswered` })
              }
              ...
              return (
                ...
                >
                  ...
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={darkMode ? '#C4B5FD' : '#5C2ED4'} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[13px] font-semibold leading-tight" style={{ color: darkMode ? '#F9FAFB' : '#1F2937' }}>
                      Finish your application
                    </div>
                    <p className="text-[11px] mt-0.5" style={{ color: darkMode ? '#9CA3AF' : '#6B7280' }}>
                      {incomplete.length === 1 ? '1 section still needs your attention.' : `${incomplete.length} sections still need your attention.`} Tap a row to jump there.
                    </p>
                    <ul className="mt-2 text-[12px] leading-relaxed space-y-1">
                      {incomplete.map(item => (
                        <li key={item.id}>
                          <button
                            type="button"
                            onClick={() => goToStep(item.id)}
                            className="text-left w-full flex items-start gap-2 rounded-lg px-2 py-1.5 transition"
                            style={{ color: darkMode ? '#D1D5DB' : '#4B5563', background: 'transparent' }}
                          ...
                          >
                            ...
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={darkMode ? '#C4B5FD' : '#5C2ED4'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 mt-1">
