    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 overflow-y-auto space-y-3">
          ...
          {/* Top summary card — same visual treatment as the BopSubmission
              page summary card. Gradient stripe, check icon, title, and
              a 3-column meta grid. */}
          {(() => {
            const productType = formData.pageZero?.productType || 'bop'
            ...
            const packageLabel = formData.bind?.packageId ? PACKAGE_LABEL[formData.bind.packageId] : null
            const subtitle = carrier
              ? <>Ready to bind with <span className="font-semibold" style={{ color: isDark ? '#F9FAFB' : '#374151' }}>{carrier}</span>{packageLabel ? <> · <span className="font-semibold">{packageLabel}</span> package</> : null}.</>
              : variant === 'download'
              ...
              >
                ...
                <div className="flex items-start gap-4 px-6 pt-5 pb-4">
                  ...
                  >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24">
                      <defs>
                        <linearGradient id={`prevCheckG-${variant}`} x1="0%" y1="0%" x2="100%" y2="0%">
                          <stop offset="0%"   stopColor={isDark ? '#A78BFA' : '#5C2ED4'}/>
                          <stop offset="100%" stopColor={isDark ? '#E879F9' : '#A614C3'}/>
                        </linearGradient>
                      </defs>
                      <path d="M5 13l4 4L19 7" stroke={`url(#prevCheckG-${variant})`} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs leading-relaxed" style={{ color: isDark ? '#9CA3AF' : '#9CA3AF' }}>
                      {subtitle}
                    </p>
                  </div>
                </div>
                ...
                >
                  {[
                    { label: 'Applicant',  value: biz.name || '—' },
                    { label: 'Class',      value: ss.naics ? `NAICS ${ss.naics}` : (ss.description || '—'), gradient: true },
                    { label: 'Effective',  value: biz.effectiveDate || '—' },
                  ].map((item, i) => (
                    <div
                      key={item.label}
                      className="px-5 py-4"
